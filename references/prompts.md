...
> **为什么必须先问**：源材料和参考风格直接影响内容提炼方向和预览图生成质量。跳过此步骤会导致后续生成偏离用户需求。
...
在生成最终PPT文件前，请先使用最新的image2图像生成模型，生成一张2×2网格合成图，包含四套不同风格的PPT缩略预览（类似幻灯片浏览视图布局），方便一次性对比查看。四套方案的内容结构保持一致，但主题颜色、排版方式和视觉风格需要有所区别，同时都要符合课堂汇报专业、美观、简洁的要求。
...
- 优先生成1张2×2合成图（4套风格在同一张图中），而非4张独立图片，减少交互摩擦

**环境适配指引（生成预览图后如何展示给用户）：**

| 环境类型 | 展示方式 |
|---------|---------|
| 有原生图片展示（GPT/ChatGPT） | 直接生成，用户在对话中即可查看 |
| 无原生图片展示（TraeWork/Claude/WorkBuddy 等） | 生成后将图片保存到用户工作目录，提供 computer:// 链接，并用明确话术引导用户点击查看（如"请点击以下链接查看4套风格预览：[链接]"） |
