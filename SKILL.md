...
深入阅读并理解源材料，提炼核心内容；同时生成四张不同风格的预览图供用户对比选择。
...
3. **提炼内容大纲**：从材料中提取以下要素：
   ...
   - 典型案例、经典语句
   - 现实意义与阅读启示
4. **生成四张预览图**：按 `references/prompts.md` 中的 **Prompt 1** 分别生成**四张不同风格的 PPT 预览图**。四套方案内容结构一致，但主题颜色、排版方式和视觉风格需有区别。如用户提供了参考模板，四套风格应在参考模板基础上做变化。
5. **展示预览并等待选择**：展示四张预览图，请用户选定其中一套风格。展示方式需根据环境适配（详见下方适配说明）。
