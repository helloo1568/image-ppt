...
2. **提炼内容大纲**：从材料中提取以下要素：
   ...
   - 典型案例、经典语句
   - 现实意义与阅读启示
3. **获取参考风格**：询问用户是否有参考 PPT 模板图片。如有，分析该图片的配色、字体、版式风格。如用户已提供参考模板，可跳过风格选择步骤，直接基于模板风格生成（降级方案，减少一步交互）。
4. **生成2×2网格预览**：使用 AI 的图像生成能力，按 `references/prompts.md` 中的 **Prompt 1** 生成**一张 2×2 网格合成图**，包含四套不同风格的 PPT 缩略预览（类似幻灯片浏览视图布局）。优先生成1张合成图而非4张独立图片，减少 API 调用次数与交互摩擦。四套方案内容结构一致，但主题颜色、排版方式和视觉风格需有区别。
...
- **降级方案**：如果用户已提供参考模板图片，可跳过4套风格选择步骤，直接基于模板风格生成，减少一步交互。
