...
3. **提炼内容大纲**：从材料中提取以下要素：
   ...
   - 典型案例、经典语句
   - 现实意义与阅读启示
4. **生成2×2网格预览**：按 `references/prompts.md` 中的 **Prompt 1** 生成**一张 2×2 网格合成图**，包含四套不同风格的 PPT 缩略预览（类似幻灯片浏览视图布局）。优先生成1张合成图而非4张独立图片，减少 API 调用次数与交互摩擦。四套方案内容结构一致，但主题颜色、排版方式和视觉风格需有区别。如用户提供了参考模板，四套风格应在参考模板基础上做变化。
...
- **通过外部工具生图、无原生图片展示的 AI（如 TraeWork/Claude/WorkBuddy）**：先用文档读取能力提炼内容，再调用可用的生图工具生成预览图。生成后需将图片保存到用户工作目录，提供 `computer://` 链接并用明确话术引导用户点击查看（如"请点击以下链接查看4套风格预览：[链接]"）。Prompt 1 中的"image2图像生成模型"替换为实际使用的生图工具。
